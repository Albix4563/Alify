...
export function Sidebar({ currentView, setCurrentView, currentPlaylist, setCurrentPlaylist, setCreatePlaylistDialog }: any) {
  ...
  return (
    <aside className="w-64 bg-white/5 backdrop-blur-2xl hidden md:flex flex-col border-r border-white/10 h-full z-10 flex-shrink-0 relative">
      <div className="absolute inset-y-0 right-0 w-[1px] bg-gradient-to-b from-transparent via-blue-500/20 to-transparent"></div>
      
      <div className="p-6 pb-2 overflow-y-auto flex-1 custom-scrollbar z-10">
        <div className="flex items-center gap-2.5 mb-8 cursor-pointer" onClick={() => setCurrentView('home')}>
      ...
      <div className="p-4 pt-2 z-10 backdrop-blur-md bg-white/5 border-t border-white/10">
        <div className="bg-black/20 backdrop-blur-xl border border-white/10 rounded-2xl p-4 cursor-pointer hover:bg-white/10 transition-colors flex flex-col gap-1 shadow-lg">
          ...
          <button onClick={() => { logout(); toast.info('Disconnesso con successo!'); }} className="w-full py-1.5 bg-gradient-to-r from-blue-700 to-sky-600 text-white text-[11px] font-bold rounded-full transition-all">Disconnettiti</button>
        </div>
      </div>
    </aside>
  );
