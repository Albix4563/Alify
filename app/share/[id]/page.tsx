...
import { doc, getDoc, collection, getDocs, query, updateDoc, arrayUnion } from 'firebase/firestore';
import { Play, ListMusic, Loader2, Users } from 'lucide-react';
...
export default function SharedPlaylist({ params }: { params: Promise<{ id: string }> }) {
    ...
    const joinPlaylist = async () => {
        ...
        try {
            ...
            setPlaylist({ ...playlist, collaborators: [...(playlist.collaborators || []), user.uid] });
            toast.success("Ti sei unito alla playlist collaborativa!");
        } catch (e: any) {
            console.error(e);
            toast.error(e.message || "Errore durante l'ingresso nella playlist");
        } finally {
            setJoining(false);
        }
    };
    ...
    const isCollaborative = playlist.isCollaborative;
    const isOwner = playlist.ownerId === user?.uid;
    const isCollaborator = playlist.collaborators?.includes(user?.uid) || false;
    const canJoin = isCollaborative && !isOwner && !isCollaborator;
        ...
        <div className="flex flex-col h-[100dvh] w-full bg-transparent text-white overflow-hidden font-sans relative z-10">
            <div className="flex-1 overflow-y-auto overflow-x-hidden overscroll-y-contain pb-24 p-8 max-w-5xl mx-auto w-full relative z-0">
                <div className="flex items-end gap-6 mb-8 mt-4 relative z-0">
                   ...
                   <div className="pb-2">
                       <div className="text-xs font-bold uppercase tracking-widest text-blue-200/60 mb-2 drop-shadow-sm flex items-center gap-2">
                         {isCollaborative ? "Playlist Collaborativa" : "Playlist Condivisa"}
                         {isCollaborative && <span className="bg-purple-500/80 text-white text-[9px] uppercase px-2 py-0.5 rounded-full font-bold shadow-md">Collab</span>}
                       </div>
                ...
                <div className="flex items-center gap-4 mb-6 relative z-0">
                    ...
                    {canJoin && (
                        <button
                          ...
                          disabled={joining}
                          className="flex items-center gap-2 px-6 h-12 bg-white/5 border border-white/10 hover:bg-white/10 rounded-full font-bold text-white transition-all hover:-translate-y-0.5 hover:shadow-lg disabled:opacity-50"
                        >
